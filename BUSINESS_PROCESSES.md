...
flowchart TD
    ...
    Y --> Z{LLM доступен?}
    Z -->|Да| AA[LLM guidance + детальный анализ]
...
flowchart LR
    ...
    E --> F
    F --> G[weighted_sum = Σ(sentiment × weight)]
    F --> H[total_weight = Σ(weight)]
    
    G --> I[weighted_sentiment = weighted_sum / total_weight]
    H --> I
...
flowchart TD
    ...
    K --> L[Расчет количества: floor(allocation / price)]
    L --> M[Расчет комиссии: notional × 0.1%]
...
flowchart TD
    ...
    F --> G[Расчет лог-доходности: log(current_price / entry_price)]
    G --> H[Порог стоп-лосса: log(0.95) ≈ -0.0513]
...
flowchart LR
    ...
    B --> C[Расчет: log(current / entry)]
    C --> D{log_return <= -0.0513?}
    D -->|Да| E[🛑 Стоп-лосс]
